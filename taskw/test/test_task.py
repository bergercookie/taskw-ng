...
class TestTaskMarshalling(TestCase):
    ...
    def test_deserialize(self):
        ...
        arbitrary_due_date = datetime.datetime.now().replace(
            tzinfo=pytz.UTC
        ) + datetime.timedelta(hours=1)
        arbitrary_tags = [
            "one",
        ...
        self.assertAlmostEqual(
            task["due"], arbitrary_due_date, delta=datetime.timedelta(seconds=1)
        )
    ...
    def test_from_input(self):
        ...
        assert on_add_task.get("entry") == datetime.datetime(
            2018, 6, 18, 3, 2, 42, tzinfo=tzutc()
        )
        ...
        assert on_modify_task.get("entry") == datetime.datetime(
            2018, 6, 18, 3, 2, 42, tzinfo=tzutc()
        )
